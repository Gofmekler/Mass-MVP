"""
MassLab_v5.py — исправленная версия
Максимально интерактивный учебный симулятор масс-спектрометра
...
- Динамические панели управления для вкладок
- Темная тема, цветовая маркировка, упражнения
"""
...
import matplotlib
matplotlib.use("QtAgg")  # <- backend должен быть указан до импорта pyplot
...
class MassLabApp(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MassLab v5 — учебный симулятор")
    ...
    def generate_new_unknown(self):
        ...
        self.statusBar().showMessage(f"Сгенерирован новый неизвестный элемент")

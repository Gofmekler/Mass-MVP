"""
MassLab_v6.py — обновленная учебная версия
- Спектр: hover, click по пикам, проверка неизвестного элемента
- Траектория: дуги радиусов кривизны, hover-аннотации, интерактивные слайдеры
- Упражнение: определить неизвестный элемент по радиусу кривизны
- Формула R = 1/B * sqrt(2*m*U/q) отображается
- Цвета: magenta, cyan, orange, red (без желтого)
"""
...
class MassLabApp(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MassLab v6 — учебный симулятор")
    ...
    def _init_ui(self):
        ...
        self.btn_detect = QtWidgets.QPushButton("Найти неизвестный элемент")
        ...
        self.input_unknown = QtWidgets.QLineEdit()
        self.btn_check_unknown = QtWidgets.QPushButton("Проверить")
        traj_layout.addWidget(self.input_unknown)
        traj_layout.addWidget(self.btn_check_unknown)
        ...
        self.btn_detect.clicked.connect(self.check_unknown)
        self.panel_spec.canvas.mpl_connect("motion_notify_event", self.on_hover_spec)
        self.panel_spec.canvas.mpl_connect("button_press_event", self.on_click_spec)
        self.btn_check_unknown.clicked.connect(self.check_unknown_traj)
    ...
    def generate_new_unknown(self):
        ...
        self.statusBar().showMessage(f"Сгенерирован новый неизвестный элемент: {self.unknown_name}")
    ...
    def on_click_spec(self,event):
        if event.inaxes!=self.panel_spec.ax: return
        x = event.xdata
        guessed = None
        if abs(x - tof_time_for_mz(self.current_unknown)) < 0.001:
            guessed = self.unknown_name
            self.statusBar().showMessage(f"✅ Верно! Это {guessed}")
        else:
            guessed = "Неверно"
            self.statusBar().showMessage(f"❌ Неверно! Сгенерирован новый элемент")
            self.generate_new_unknown()
        self.update_plots()
    ...
    def check_unknown_traj(self):
        user_input = self.input_unknown.text().strip()
    ...
    def update_plots(self):
        if self.tabs.currentWidget()==self.tab_spectrum:
            active_cal = CALIBRANTS
            mz_list = [ELEMENTS[n] for n in active_cal]+[self.current_unknown]
            ...
            self.lbl_formula.setText(f"<b>Формула радиуса кривизны:</b><br>R = 1/B * sqrt(2 * m * U / q)<br>"
                                     f"Где U={self.U:.0f} V, B={self.B:.3f} T")
